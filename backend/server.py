...
async def get_property_management_status():
    ...
    return {
        ...
        "message": "Property management features are active" if PROPERTY_MANAGEMENT_ENABLED else "Property management features are disabled"
    }
...
async def create_device(property_id: str, device_data: dict, user_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        response_dict = device_dict.copy()
        response_dict.pop("_id", None)  # Remove _id if it exists
        
        return {
            "device_id": device_dict["id"],
            "message": "Device created successfully",
            "device": response_dict
        }
...
# Add database initialization for new collections
@app.on_event("startup")
async def startup_property_db():
    """Initialize property management database indexes"""
    try:
        if PROPERTY_MANAGEMENT_ENABLED:
            ...
            print("✅ Property management database initialized successfully")
    except Exception as e:
